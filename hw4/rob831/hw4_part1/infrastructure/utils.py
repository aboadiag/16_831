        ...
        if render:  # feel free to ignore this for now
            if 'rgb_array' in render:
                if hasattr(env.unwrapped, 'sim'):
                    ...
                    image_obs.append(env.render(mode=render))
            if 'human' in render:
...
def normalize(data, mean, std, eps=1e-8):
    ...
    # if np.all(std == 0):  # Handle zero std
    #     print("Warning: Standard deviation is zero, skipping normalization.")
